...
namespace ega
{
    ...
    public:
        ...
        static constexpr size_t MX_INS_NUM = 6; ///< 最大支持的 UART 实例数，实例需要与外设一一对应
        static constexpr size_t MX_RX_BUFFER_SIZE = 256; ///< 接收缓冲区大小，默认256字节
    ...
    public:
        ...
        struct Config
        {
            ...
            uint16_t rx_size = 0;
            bool rx_auto_restart = true; ///< 是否自动重启接收
     ...
     */
        uint16_t receive(uint8_t* data, uint16_t target_size, uint32_t timeout = HAL_MAX_DELAY);
        /**
     ...
     */
        void setReceiveType(Rx_Type type, uint16_t size);
        /**
    ...
    private:
        ...
        static std::array<UARTInstance*, MX_INS_NUM> instance_list_;
        static uint8_t instance_count_;

        UART_HandleTypeDef* handle_; ///< HAL UART 句柄

        uint16_t rx_size_; ///< 接收数据长度
        uint8_t rx_buff_[MX_RX_BUFFER_SIZE]{0}; ///< 接收缓冲区
        Rx_Type rx_type_; ///< 接收模式
        RxCallback rx_callback_; ///< 接收回调
        bool rx_auto_restart_ = true; ///< 是否自动重启接收
