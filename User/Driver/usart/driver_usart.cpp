...
{
    /// 静态成员初始化
    std::array<UARTInstance*, UARTInstance::MX_INS_NUM> UARTInstance::instance_list_{};
    ...
    UARTInstance::UARTInstance(const Config& config)
    {
        ...
        rx_auto_restart_ = config.rx_auto_restart;
    ...
    uint16_t UARTInstance::receive(uint8_t* data, uint16_t target_size, uint32_t timeout)
    {
    ...
    void UARTInstance::stopReceive()
    {
        HAL_UART_AbortReceive(handle_);
        memset(rx_buff_, 0, MX_RX_BUFFER_SIZE);
        //WARNING("[UARTStopRecv] 接收已停止");
    }

    void UARTInstance::startReceive()
    {
        switch (rx_type_)
        {
        case DMA_IDLE:
    ...
    void UARTInstance::setReceiveType(Rx_Type type, uint16_t size)
    {
        rx_type_ = type;
        rx_size_ = size;
        stopReceive();
        if (type != BLOCK_IDLE && type != BLOCK_NUM)
            startReceive(); // 非阻塞模式,重启接收服务
    }
...
{
    for (auto ins : ega::UARTInstance::instance_list_)
    {
        if (ins->handle_ == huart && ins->rx_callback_ != nullptr)
        {
            ins->rx_callback_(ins->rx_buff_, Size);
            if (ins->rx_auto_restart_)
            {
                ins->startReceive();
            }
            break;
...
{
    for (uint8_t i = 0; i < ega::UARTInstance::instance_count_; i++)
    {
        auto ins = ega::UARTInstance::instance_list_[i];
        if (ins && ins->handle_ == huart)
        {
            ...
            if (ins->tx_use_fifo_ && ins->tx_queue_.size())
            {
                ...
                for (uint8_t i = 0; i < ega::UARTInstance::COPY_POOL_BLOCKS; i++)
                {
...
void HAL_UART_ErrorCallback(UART_HandleTypeDef* huart)
{
    for (auto ins : ega::UARTInstance::instance_list_)
    {
        if (ins->handle_ == huart)
        {
            ...
            if (ins->rx_type_ != ega::UARTInstance::BLOCK_IDLE &&
                ins->rx_type_ != ega::UARTInstance::BLOCK_NUM)
            {
                ins->startReceive();
            }
        }
