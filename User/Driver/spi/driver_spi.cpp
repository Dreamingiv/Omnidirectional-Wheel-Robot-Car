...
{
    /// 静态成员初始化
    std::array<SPIInstance*, SPIInstance::MX_INS_NUM> SPIInstance::instance_list_{};
    ...
    SPIInstance::SPIInstance(const Config& config)
    {
        ...
        this->rx_callback_ = config.rx_callback;
        this->tx_callback_ = config.tx_callback;
        // 加入实例列表
    ...
    SPIInstance::TxRx_State SPIInstance::sendAndReceive(uint8_t* tx_data, uint16_t size, uint32_t timeout)
    {
    ...
    SPIInstance::TxRx_State SPIInstance::sendAndReceive(uint8_t* tx_data, uint8_t* rx_data, uint16_t size,
                                                        uint32_t timeout)
    {
        if (mode_ == TX_ONLY || mode_ == RX_ONLY) { return ERROR_STATE; }
    ...
    SPIInstance::TxRx_State SPIInstance::send(uint8_t* tx_data, uint16_t size, uint32_t timeout)
    {
    ...
    SPIInstance::TxRx_State SPIInstance::receive(uint16_t size, uint32_t timeout)
    {
    ...
    void SPIInstance::setSendAndReceiveType(TxRx_Type type, uint16_t size)
    {
        stop();
        type_ = type;
        tx_size_ = size;
...
{
    for (auto& ins : ega::SPIInstance::instance_list_)
    {
        if (ins->handle_ == hspi && ins->tx_callback_ != nullptr)
        {
            ins->tx_callback_();
            ins->csDisable();
        }
...
void HAL_SPI_RxCpltCallback(SPI_HandleTypeDef* hspi)
{
    for (auto& ins : ega::SPIInstance::instance_list_)
    {
        if (ins->handle_ == hspi && ins->rx_callback_ != nullptr)
        {
            ins->rx_callback_(ins->rx_buff_, ins->rx_size_);
            ins->csDisable();
        }
    }
}
