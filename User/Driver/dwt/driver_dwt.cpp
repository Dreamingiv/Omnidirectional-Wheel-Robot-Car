...
namespace ega
{
    ...
    void DWTInstance::SysTimeUpdate()
    {
        volatile uint32_t cnt_now = DWT->CYCCNT;
        ...
        time.s = CNT_TEMP1;
        time.ms = CNT_TEMP2 / CPU_FREQ_Hz_ms;
        CNT_TEMP3 = CNT_TEMP2 - time.ms * CPU_FREQ_Hz_ms;
        time.us = CNT_TEMP3 / CPU_FREQ_Hz_us;
    }
    ...
    float DWTInstance::getTimeline_s()
    {
        DWTInstance& instance = getInstance();
        instance.SysTimeUpdate();

        float DWT_Timelinef32 = instance.time.s + instance.time.ms * 0.001f + instance.time.us * 0.000001f;

        return DWT_Timelinef32;
    }

    float DWTInstance::getTimeline_ms()
    {
        DWTInstance& instance = getInstance();
        instance.SysTimeUpdate();

        float DWT_Timelinef32 = instance.time.s * 1000 + instance.time.ms + instance.time.us * 0.001f;

        return DWT_Timelinef32;
    }

    uint64_t DWTInstance::getTimeline_us()
    {
        DWTInstance& instance = getInstance();
        instance.SysTimeUpdate();
        return instance.time.s * 1000000 + instance.time.ms * 1000 + instance.time.us;
    }
    ...
    float DWTInstance::getDeltaT()
    {
        DWTInstance& instance = getInstance();
        volatile uint32_t cnt_now = DWT->CYCCNT;
        float dt = ((uint32_t)(cnt_now - instance.cnt_)) / ((float)(instance.CPU_FREQ_Hz));
        instance.cnt_ = cnt_now;
    ...
    double DWTInstance::getDeltaT64()
    {
        DWTInstance& instance = getInstance();
        volatile uint32_t cnt_now = DWT->CYCCNT;
        double dt = ((uint32_t)(cnt_now - instance.cnt_)) / ((double)(instance.CPU_FREQ_Hz));
        instance.cnt_ = cnt_now;
