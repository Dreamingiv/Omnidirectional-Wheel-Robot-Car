...
namespace ega
{
    ...
    void VT13::debugPrintKeys()
    {
        ...
        const auto& k = ins.rc_current_.key[KEY_PRESS];
        logger_printf(
            "[VT13 Keys] W=%d S=%d A=%d D=%d Shift=%d Ctrl=%d Q=%d E=%d R=%d F=%d G=%d Z=%d X=%d C=%d V=%d B=%d\r\n",
    ...
    {
        auto & ins = getInstance();
        const auto& kc = ins.rc_current_.key[KEY_PRESS_WITH_CTRL];
        const auto& ks = ins.rc_current_.key[KEY_PRESS_WITH_SHIFT];
