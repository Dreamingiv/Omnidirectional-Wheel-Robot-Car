...
namespace ega
{
    ...
    float PID::calculate(float target, float measure, float forward) {
        ...
        if (use_ramp_)
        {
            target_ = utils::limit(target, last_target_ - ramp_step_ * dt_, last_target_ + ramp_step_ * dt_);
        }
        else
        {
            target_ = target;
        ...
        // }
        // ✅ 输出限幅
        output_ = utils::limit(output_, -limit_output_, limit_output_);
